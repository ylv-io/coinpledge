import { getWeb3js, getCoinContractPromise, getAccount, fromWei } from './web3';
import { arrayToChallenge } from '../../utils/web3';

import { addUserChallenge, updateUserChallenge } from '../../actions/userChallenges';
import { addMentorChallenge, updateMentorChallenge } from '../../actions/mentorChallenges';
...
export const getChallengesForUser = async (user) => {
  ...
  if(numberOfChallenges > 0) {
    ...
    const challenges = challengesRaw.map((o, i) => arrayToChallenge(o, indexes[i].toNumber(), account));
    return challenges;
  }
  return [];
}
...
export const subscribeToCoinEvents = async (store) => {
  ...
  newChallengeEvent.watch(function(error, result) {
    ...
    if (!error) {
      ...
      if(user === account) {
        ...
        if(!exst)
          store.dispatch(addUserChallenge(newChallenge));  
      }
      if(mentor === mentor) {
        const exst = store.getState().mentorChallenges.filter(o => o.id === newChallenge.id)[0];
        if(!exst)
          store.dispatch(addMentorChallenge(newChallenge));
      }
    } else {
      console.log(error);
    }
  ...
  challengeResolvedEvent.watch(function(error, result) {
    if (!error) {
      const { challengeId, user, mentor, decision } = result.args;
      const updates = {
        successed: decision,
        resolved: true,
        canResolve: false,
      ...
      if(user === account) {
        ...
        if(!exst)
          store.dispatch(updateUserChallenge(challengeId.toNumber(), updates));
      }
      if(mentor === mentor) {
        const exst = store.getState().mentorChallenges.filter(o => o.id === challengeId)[0];
        if(!exst)
          store.dispatch(updateMentorChallenge(challengeId.toNumber(), updates));
      }
    } else {
      console.log(error);
    }
  ...
  bonusFundChangedEvent.watch(function(error, result) {
    if (!error) {
      const { user, value } = result.args;
      if(user === account)
        store.dispatch(setBonusFund(fromWei(value.toNumber(), 'ether')));
    } else {
      console.log(error);
    }
  });
